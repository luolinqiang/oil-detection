...
//        getAddressByIp();
        $('#q_position_text').val('读取失败');
    });
}
var getAddressByLL = function(pos){
    $.ajax({
        url : '/geo/l2a?lat=' + pos.coords.latitude + "&lng=" + pos.coords.longitude,
        dataType : 'json',
        success  : function(data){
            if(0 == data.code){
                if($('#q_position_text').val()=='获取中') {
                    $('#q_position_text').val(data.result.province + "-" + data.result.city);
//                    $('#q_position').val();
                }
            }else{
                if($('#q_position_text').val()=='获取中')
...
var htmlTemplate =
    '<li class="am-list-item-thumb-left list-item card" >' +
        ...
        '<a href="/item-{{id}}" class=""><img src="{{imageUrl}}" alt="{{titleAlt}}"/></a>' +
        '</div>' +
        '<div class=" am-u-sm-9 am-list-main"  >' +
        '<div class="am-list-item-hd">' +
        '<a href="/item-{{id}}" class=""><div class="am-u-sm-12 text-overflow-ellipsis">{{title}}</div></a>' +
        '</div>' +
