...
template.helper('$parseTime',function(c){
    return new Date((c*1000)).formatDate('yyyy-mm-dd hh:mm:ss');
});
template.helper('$fixed',function(c,l){
    return parseFloat(c).toFixed(l);
});
template.helper('$urldecode',function(c){
    return decodeURIComponent(c);
});
template.helper('$image',function(c,s){
    return c.replace(/\{X\*Y\}/g,s);
//    return '';
});